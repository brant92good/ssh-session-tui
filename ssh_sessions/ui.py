...
from .catalog import Machine, Route, Snapshot, address, label, login, new_id, port_number
...
class Picker(App[Choice]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id='top'):
            ...
            yield Static('Open a remote machine or a local terminal. Private keys stay on this device.', id='intro', classes='muted', markup=False)
    ...
    def on_mount(self):
        ...
        self.query_one('#machines', DataTable).add_columns('Key', 'Machine / terminal', 'Route on this device', 'Address', 'Username')
        self.action_reload()
    ...
    def refresh_rows(self):
        ...
        for target in self.row_targets:
            ...
            table.add_row(keys.get(machine.id, ''), Text(machine.name), Text(route.name if route else 'Choose a route'),
